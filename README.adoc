...
The Tribestream API Gateway will help you to easily configure OAuth2 to protect your microservices using the OAuth2 Profile. TAG will authenticate you using your credentials to issue an access token. This token can be used to call a protected route that will have access to the back end microservice.
...
TAG emits the access tokens using the JWT format. The emitted token is a base64 hash of a JSON that has a header, a payload with the claims and the signature to make sure the token was not modified. TAG will receive the JWT access tokens to authorize incoming requests to existing routes.

...
After that click _Test_. You will get back a 403 and the reason is that the client must have the role _USA_ to be authorized to access the route.

image::test-window-with-oauth2-403.png[""]

So now, scroll to the top and let's save the test scenario _Movies Catalog_ clicking
...
Then go to the route _Movies Catalog Route_, open the _Test Routes_ window and search for _Movies Catalog_. You will get saved information back, now click _Test_.

You will now receive a _200_ in the response, which means you were able to get a token, you used this token to call the _/movies-catalog_, and you were authorized to use the route successfully as you had the necessary role to do it. Therefore the request will be proxied and the microservice will return te catalog of movies for the Streaming Website.
